...
class Bookings extends CI_Controller {
           ...
           // echo '<pre>';
                       // print_r($data);exit;
						 ...
						 if($last_insert_id) {
						 	...
						 	if($this->session->userdata(USER_SESSION.'is_logged_in') == true && $this->session->userdata(USER_SESSION.'user_type') == 'AGENT'){
						 	...
						 	}else if($this->session->userdata(USER_SESSION.'is_logged_in') == true && $this->session->userdata(USER_SESSION.'user_type') == 'USER'){
						 			$this->hotel_booking_by_user($last_insert_id,$inputArray['total_amount']);
						 		}else{
						 		...
						 		if($this->input->post('user_type') == 'GUEST'){
							        ...
							        $this->master_db->update(CUST_HOTEL_BOOKING_DETAILS,$dataArr,array("cust_id"=>$last_insert_id));
									//booking pay Hotel
									...
									$post_data['payment_type'] = 'deposit';
									$post_data['payref'] ='payref';
									$post_data['session_id'] = $arr_booking_dtls->session_id;

									$post_data['room_code'] = $arr_booking_dtls->room_code;
									$post_data['number_of_room'] = $arr_booking_dtls->no_of_rooms;
									$post_data['guess_phone'] = $arr_booking_dtls->cust_phone;
									$post_data['guess_email'] =  $arr_booking_dtls->cust_email;
									$post_data['room_name_1'] =  $arr_booking_dtls->room_name;
									$post_data['room_smooking_1'] = 'no';
									$post_data['room_request_1'] ='no';
									$post_data['room_name_2'] = $arr_booking_dtls->room_name;
									$post_data['room_smooking_2'] = 'no';
									$post_data['room_request_2'] ='no';

									/*$inputArray['session_id'] = $arr_booking_dtls->session_id;
									$inputArray['payment_type'] = 'deposit';
									$inputArray['payref'] = 'payref';
									$inputArray['room_code'] = 
									$inputArray['number_of_room'] = 
									$inputArray['guest_phone'] = 
									$inputArray['guest_email'] =
									$inputArray['room_name_1'] = */
									

									
									//$userinfo = $this->master_db->select('user_id,wallet_balance',USERS,array('user_id'=>$last_insert_id)));
									...
									$result = getDataFromRemot('hotel','hotel_pay','','',$post_data); 
								//	print_r($result);exit;
									if(isset($result->error_msg) && ($result->error_msg != '')){
										//echo '<pre>';
										//http://exceptionaire.co/rumbostar/hotel-booking/_omXwloP-geDZYclyKkegH64ofTFk539WyNXw5o8ERs/room_code_0
										//print_r($arr_booking_dtls);
										$responce_str = site_url().'hotel-booking/'.$arr_booking_dtls->session_id.'/'.$arr_booking_dtls->room_code; 
										// print_r($result->error_msg);exit;
										$msgArr = array(
														"msg_type"=>"error",
										...
										/*$dataInsert = array(
											"amount"=>$inputArray['total_amount'],
											"transaction_type"=>'Dr',
											"final_amount"=>$inputArray['total_amount'],
								...
								$this->paypal_lib->add_field('item_name', $data['hotel_name']);
								$this->paypal_lib->add_field('custom', $data['cust_email']);
								$this->paypal_lib->add_field('item_number',  $data['cust_phone']);
								$this->paypal_lib->add_field('amount',  '23');		
								//$this->paypal_lib->image($logo); $data['booking_amount']	
								$this->paypal_lib->paypal_auto_form(); exit;*/
						 		//$this->vtweb_checkout($inputArray);die; sk comment line
    ...
    /*@author : smita
     *@function use : hotel_booking_by_user this function use for user can book hotes
     * date : 5-2-2018
     */

    public function hotel_booking_by_user($bkTblID,$amount){
				 	...
				 	if(isset($last_insert_id_depart) && $last_insert_id_depart > 0 || isset($last_insert_id_retrun) && $last_insert_id_retrun >0)
					{
                       ...
                       // $berth_preference = $inputArray['berth_preference'];
						...
						{		
							...
							//$post_data['first_name_'.$j] = $first_name[$i];
							//$post_data['last_name_'.$j] = $last_name[$i];
							$post_data['id_card_'.$j] = $this->input->post('guest_phone');//$pessagerMobNo[$i];
