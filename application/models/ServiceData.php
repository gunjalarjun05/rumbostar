...
class ServiceData extends CI_Model {
  ...
  public function get_flight_sorted_result($array,$sortBy,$min=false,$max=false){
   ...
   // echo $sortBy;
    ...
    switch ($sortBy) {
        case 1:
       // $products = $jSonDecode->schedule->depart;
           //  usort($jSonDecode->schedule->depart, 'sortJsonArr');
  ...
  */
  ...
  /* @Author : Smita
   * @Function use : checkFlightCoupon function use for check flight assign coupon 
     code or not
   * @Date : 5-2-2018
   */
  public function checkFlightCoupon($flightId){

    $this->db->select('travel_id,travel_type,user_id');
    $this->db->from(LOYALTY_USER_DISCOUNT);
    $this->db->where('travel_id', $flightId);
    $query = $this->db->get();
    //echo $this->db->last_query();exit;
    return  $query->result();
  } 
